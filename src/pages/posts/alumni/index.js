import { GLOBAL_URL, getPostsAlumni } from "@/utils/fetch";
import Link from "next/link";
import React, { useContext, useEffect, useState } from "react";
import toast from "react-hot-toast";
import categoriesPost from "@/data/alumnus/post/categories.json";
                    ...
                    <div className="row gy-5">
                        ...
                        <div className={`search-box-2 ${theme === "dark" ? "bg-light" : "" }`}>
                            <form>
                                ...
                                <button><i className={`bi bi-search ${theme === "dark" ? "text-light" : ""}`} /></button>
                            </form>

                        </div>
                        <Link href={"/posts/alumni/create-post"}>
                            ...
                            }} className="header-icons d-flex flex-row align-items-center justify-content-evenly bg-primary">
                                <div style={{

                                }} className="search-btn d-flex flex-row align-items-center"> <i className="bi bi-pen-fill" />
                                    <p className="m-2">Create Post</p>
                                </div>
                                    ...
                                    PostData && PostData.map((post, index) => {
                                        return <div key={post._id} className="col-md-6">
                                            <div className="blog-grid-1">
                                                ...
                                                {
                                                    post?.imageLink ? <Link legacyBehavior href={`/posts/alumni/id/${post._id}`}>
                                                        <a className="image">
                                                            <img src={`${GLOBAL_URL}/api/user/post/image/${post?.imageLink}`} alt="image" />
                                                        </a>
                                                    </Link> : <Link legacyBehavior href={`/posts/alumni/id/${post._id}`}>
                                                        <a className="image">
                                                ...
                                                <div className="content">
                                                    ...
                                                    <h4><Link legacyBehavior href={`/posts/alumni/id/${post._id}`}><a className={`${theme === "dark" ? "text-light text-hover" : ""}`}>{post?.title}.</a></Link></h4>
                                                    <div className="bottom-area">
                                                        <Link legacyBehavior href={`/posts/alumni/id/${post._id}`}><a className={`eg-btn arrow-btn ${theme === "dark" ? "text-light" : ""}`}>View Details<i className="bi bi-arrow-right" /></a></Link>
                                    ...
                                    <ul className="category-list">

                                        {
                                            categoriesPost?.map((category, index) => {
                                                return <li key={index} className={`${theme === "dark" ? "span-li" : ""}`}><Link legacyBehavior href="/posts/alumni">
                                                    <a className={`${theme === "dark" ? "fill-hover" : ""}`}>
