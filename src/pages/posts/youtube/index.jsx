                            ...
                            return <div key={post?._id} className="col">
                                <div className="blog-video-2 style-2">
                                    ...
                                    <div className="content">
                                        <ul>
                                            <li><Link legacyBehavior href={`/posts/youtube/id/${post?._id}`}><a className={`${theme === "dark" ? "text-light text-hover" : ""}`}>By Moderator</a></Link></li>
                                            <li><Link legacyBehavior href={`/posts/youtube/id/${post?._id}`}><a className={`${theme === "dark" ? "text-light text-hover" : ""}`}>{new Date(post?.date).toDateString()}</a></Link></li>
                                            <li className={`${theme === "dark" ? "text-light" : ""}`}>{post?.views} Views</li>
                                        </ul>
                                        <h4><Link legacyBehavior href={`/posts/youtube/id/${post?._id}`}><a className={`${theme === "dark" ? "text-light text-hover" : ""}`}>{post?.title}</a></Link>
                                        </h4>
                                        <div className="bottom-area">
                                            <Link legacyBehavior href={`/posts/youtube/id/${post?._id}`}><a className={`eg-btn arrow-btn ${theme === "dark" ? "text-light" : ""}`}>View Details<i className="bi bi-arrow-right" /></a></Link>
